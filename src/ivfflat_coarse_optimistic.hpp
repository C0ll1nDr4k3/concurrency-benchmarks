...
class IVFFlatCoarseOptimistic {
 ...
 public:
   ...
   */
  IVFFlatCoarseOptimistic(Dim dim,
                          size_t nlist = 100,
                          size_t nprobe = 1,
  ...
  /// override.
  explicit IVFFlatCoarseOptimistic(const IVFParams& p, size_t max_retries = 10)
      : IVFFlatCoarseOptimistic(p.dim, p.nlist, p.nprobe, max_retries) {
    if (p.max_count > 0)
