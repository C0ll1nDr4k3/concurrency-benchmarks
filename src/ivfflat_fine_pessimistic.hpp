...
/**
 * @brief IVFFlat index with fine-grained pessimistic locking.
 */
...
class IVFFlatFinePessimistic {
 ...
 public:
   ...
   */
  IVFFlatFinePessimistic(Dim dim, size_t nlist = 100, size_t nprobe = 1)
      : dim_(dim),
        nlist_(nlist),
        nprobe_(nprobe),
        trained_(false),
  ...
  explicit IVFFlatFinePessimistic(const IVFParams& p)
      : IVFFlatFinePessimistic(p.dim, p.nlist, p.nprobe) {
    if (p.max_count > 0)
