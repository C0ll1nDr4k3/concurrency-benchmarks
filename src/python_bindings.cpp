...
PYBIND11_MODULE(nilvec, m) {
  ...
  py::class_<ConflictStats>(m, "ConflictStats")
      ...
      .def_property_readonly("insert_attempts",
          [](const ConflictStats& s) { return s.insert_attempts.load(); })
      .def_property_readonly("insert_conflicts",
          [](const ConflictStats& s) { return s.insert_conflicts.load(); })
      .def_property_readonly("search_attempts",
          [](const ConflictStats& s) { return s.search_attempts.load(); })
      .def_property_readonly("search_conflicts",
          [](const ConflictStats& s) { return s.search_conflicts.load(); });
  ...
  auto bind_hnsw = [&](auto tag, const std::string& name, bool has_stats = false) {
    using Index = typename decltype(tag)::type;
    auto cls = py::class_<Index>(m, name.c_str())
        .def(py::init<Dim, size_t, size_t, float>(),
             py::arg("dim"),
             py::arg("M") = 16,
    ...
    if (has_stats) {
        // We need to use a lambda or similar if the method might not exist in template instantiation, 
        // but here we know these specific classes have it or we wouldn't set has_stats.
        // However, to be safe with templates in C++, we use if constexpr in a lambda.
        cls.def("conflict_stats", [](const Index& self) -> const ConflictStats& {
  ...
  auto bind_ivfflat = [&](auto tag, const std::string& name, bool has_stats = false) {
    using Index = typename decltype(tag)::type;
    auto cls = py::class_<Index>(m, name.c_str())
        .def(py::init<Dim, size_t, size_t>(),
             py::arg("dim"),
             py::arg("nlist") = 100,
             py::arg("nprobe") = 1)
        .def("train",
  ...
  bind_ivfflat(std::type_identity<IVFFlatVanilla<float>>{}, "IVFFlatVanilla", false);
